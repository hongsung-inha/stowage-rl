# ══════════════════════════════════════════════════════════════════════
#  stowage_core.py  (auto-extracted from VESSL notebook Cell 4 + Cell 6)
#  · 환경/보상(R1~R15)/학습함수/시각화/export 정의 + CONFIG/POLICIES
#  · OUTPUT_BASE / RESULTS_DIR 는 train_ppo.py 가 import 전에 env 로 주입하거나
#    이 모듈이 직접 결정 (VESSL_OUTPUT_DIR → /shared → ./results_vessl)
# ══════════════════════════════════════════════════════════════════════
import os as _os_boot
OUTPUT_BASE = _os_boot.environ.get(
    'VESSL_OUTPUT_DIR',
    _os_boot.environ.get('OUTPUT_BASE', '/shared')
)
try:
...
print(f'[stowage_core] OUTPUT_BASE = {OUTPUT_BASE}')
print(f'[stowage_core] RESULTS_DIR = {RESULTS_DIR}')
...
def setup_korean_font() -> str:
    ...
    · fc-cache/apt-get 이 없는 컨테이너(예: 일부 VESSL 이미지)에서도
      예외로 죽지 않고 DejaVu Sans 로 폴백한다 (학습엔 영향 없음)."""
    try:
        subprocess.run(["apt-get", "install", "-y", "fonts-nanum"], capture_output=True)
        subprocess.run(["fc-cache", "-fv"], capture_output=True)
    except (FileNotFoundError, Exception):
        pass  # 폰트 도구 없음 — 한글 라벨이 깨질 수 있으나 결과·수치엔 무영향
...
def _build_ppo_model(env, config: dict, seed: int):
    ...
    return PPO(
        ...
        seed           = seed,
        tensorboard_log = os.path.join(RESULTS_DIR, 'tb', config.get('experiment_name', 'ppo')),
        verbose        = 0,
        device         = "auto",
    )
...
def make_phase_weights(base_w: dict, mode: str) -> dict:
    ...
    Args:
        ...
        mode  :
          ...
          'ESSENTIAL_STABILITY'  — ② 필수 + 안정 그룹만 활성
          'ESSENTIAL_EFFICIENCY' — ③ 필수 + 효율 그룹만 활성
          'BASE'                 — ④ 필수 + 안정 + 효율 전체 활성
          (legacy alias 호환: 'STABILITY_ONLY'→②, 'EFFICIENCY_ONLY'→③)

    Returns:
        modified weight dict (same keys, 비활성 그룹은 0.0)
    """
...
POLICIES = {
    ...
    "SF": {
        "desc":          "Safety-First (필수 + 안정 R5,R6, all rounds)",
        "kor":           "② 필수 + 안정 그룹만 학습",
        "color":         "#2E75B6",   # blue
        "marker":        "o",
        "phase1_rounds": [1, 2],
    ...
    "EF": {
        "desc":          "Efficiency-First (필수 + 효율 R4,R8,R10,R12,R13,R14,R15, all rounds)",
        "kor":           "③ 필수 + 효율 그룹만 학습",
        "color":         "#C00000",   # red
        "marker":        "s",
        "phase1_rounds": [1, 2],
