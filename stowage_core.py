# ══════════════════════════════════════════════════════════════════
#  stowage_core.py — VESSL 병렬 실행용 공유 모듈
#  (원본 노트북 Cell 2(정의) + Cell 3(CONFIG/POLICIES) 를 그대로 모듈화)
#
#  · 환경변수 OUTPUT_ROOT 로 결과 저장 루트를 지정 (VESSL: /output)
#  · 이 파일은 import 시 부수효과(학습)를 발생시키지 않는다.
#    (원본 셀 말미의 print/검증 출력은 보존 — import 시 1회 출력됨)
# ══════════════════════════════════════════════════════════════════
...
print("✅ [A] Imports & seed fixed (GLOBAL_SEED=42) — PPO only, v16")
...
# ─────────────────────────────────────────────────────────────────────
# SECTION C: 적재 환경 — SingleBayStowageEnv (v16 보상 구조)
#
#  ★ POD 6개, 커리큘럼 4단계 (4×4→6×6→8×8→10×10)
#  ★ MAX_ROWS = 10, OBS_DIM = 79
#
#  ★★★ 보상 구조 v16 (안정:효율 50:50) ★★★
#
#  [핵심 원칙]
...
#
#  [Step-level 보상]  (★ v16 가중치 / 계층: 필수·안정·효율 | 안정:효율 = 15:15 정확히 50:50)
#    R1   유효 적재 보너스              +1.0          [필수]
#    R2   스택 가득참 패널티            −10.0         [필수]
#    R3   오버스토우 패널티             −15.0         [필수]
#    R4   올바른 하역 순서 보너스       +2.0          [효율] ★ 3.0→2.0
#    R5   중량 균형 보너스              +6.5 × (1−CV) [안정] ★ v16: 6.0→6.5
#    R6   COG 편차 패널티              −8.5 × 편차비율 [안정] ★ v16: −8.0→−8.5
...
print("✅ [C] SingleBayStowageEnv defined (v16: Same-Tier Band reward)")
...
class ExperimentLogger:
    ...
    def __init__(self, name: str, config: dict):
        ...
        _root        = os.environ.get("OUTPUT_ROOT", "/content/results")
        self.log_dir = f"{_root}/{name}_{ts}"
...
def plot_learning_curves_by_round(
    ...
    fig.suptitle(
        ...
        f"(v16: Same-Tier Band reward | Single-Bay, GP-40, {N_POD} PODs, "
...
def plot_learning_curves_by_step(
    ...
    fig.suptitle(
        ...
        "(Smoothed episode reward, window=20 | v16 reward structure)",
        fontsize=11, fontweight="bold", y=1.02)
...
def plot_round_comparison(
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    fig.suptitle("Fig. 3  PPO Round-wise Performance (v16)",
...
def plot_metrics_radar(
    ...
    fig.suptitle("Fig. 4  PPO Performance Radar (v16)", fontsize=11, fontweight="bold")
...
def plot_bay_plan(model, level: int, config: dict, logger: ExperimentLogger,
    ...
    fig.suptitle(
        f"Fig. 5  Bay Plan — {algo_name} (Lv{level}, v16)\n"
...
def print_round_summary_table(ppo_res: Dict, config: dict):
    """PPO 라운드별 결과 요약 테이블."""
    print("\n" + "═" * 90)
    print("  TABLE: PPO — Round-wise Performance Summary (v16)")
    print("═" * 90)
...
def save_results_to_excel(
    ...
    for rnd_idx, bp in enumerate(ppo_res.get("per_round_bay_plans", [])):
        ...
        title_cell = ws_bp.cell(row=1, column=1,
            value=f"{algo_name} Round {rnd_num} (Lv{lv}) — {n_rows_bp}R×{n_tiers_bp}T  [v16]")
    ...
    items = [
        ("Version", "v16 (Same-Tier Band reward, 안정:효율 50:50)"),
...
def save_random_input_data_to_excel(
    ...
    title = ws.cell(row=1, column=1,
        value=f"Random Input Data — PPO v16 | seed={seed} | "
...
def print_reward_component_table(
    ...
    print("\n" + "═" * 70)
    print("  TABLE: Reward Component Breakdown — PPO (Final Round, v16)")
    print("═" * 70)
...
def plot_reward_components(
    ...
    fig.suptitle(
        "Fig. 6  PPO — Reward Component Breakdown (Final Round, v16)",
        fontsize=11, fontweight="bold")
...
print("✅ [F] All visualization & export functions defined (PPO only, v16)")
print()
print("  All sections loaded. Proceed to Cell 3 (CONFIG) → Cell 4 (Experiment).")


# ─────────── CONFIG & POLICIES (원본 Cell 3) ───────────
# ══════════════════════════════════════════════════════════════════
#  CONFIG v17: 안정:효율 비중 연속 가중 6-정책 비교 (ES / SF_9_EF_1 / SF_7_EF_3 / BL_SF_5_EF_5 / EF_7_SF_3 / EF_9_SF_1)
#   · ★★ v16 변경 (v15 대비): 안정:효율 = 50:50 정확 균형 (계수 절댓값 합 기준)
#
#  ┌─────────────────────────────────────────────────────────────┐
#  │ 1. 보상 항목 계층 분류 (3계층)                                  │
#  └─────────────────────────────────────────────────────────────┘
#       - 필수(Essential) : R1, R2, R3, R7, R9, R11  (6개, 모든 정책 항상 활성)
#       - 안정(Stability) : R5, R6                    (2개)
#       - 효율(Efficiency): R4, R8, R10, R12, R13, R14, R15  (7개, R13은 a/b 2키)
#
#  ┌─────────────────────────────────────────────────────────────┐
#  │ 2. 보상 가중치 (★ v16: 안정:효율 = 15.0 : 15.0 = 정확히 50:50)  │
#  └─────────────────────────────────────────────────────────────┘
#       균형 정의 = "계수 절댓값 합(Σ|w|)" 기준 (정책-독립적 설계 파라미터)
#       · 안정 Σ|w| = |+6.5| + |−8.5| = 15.0   (★ v15: 6.0/−8.0 → v16: 6.5/−8.5)
#       · 효율 Σ|w| = |+2.0|+|+1.33|+|+2.67|+|+2.0|+|+1.33|+|−1.0|+|−2.0|+|−2.67| = 15.0
#       · 안정 : 효율 = 15.0 : 15.0 = 50.0% : 50.0%   ✅
#       [주의] 계수 절댓값 합 = 50:50 은 "설계 의도(design knob)" 균형이며,
#              실제 누적 보상 기여도는 발동 빈도·스케일에 따라 달라질 수 있음.
#              → 실측 기여도는 Cell 10B(검증 셀)에서 정책별로 별도 측정·보고.
#       (필수 그룹은 균형 대상 아님: R7 completion=80.0×VPR 등 절대 스케일 유지)
#
#  ┌─────────────────────────────────────────────────────────────┐
#  │ 3. 정책 정의 및 커리큘럼                                        │
#  └─────────────────────────────────────────────────────────────┘
#       · 6-정책 안정:효율 비중 가중 (각 정책은 전 라운드 동일 가중치로 고정 학습)
#         필수(R1,R2,R3,R7,R9,R11)는 모든 정책 동일. 안정(R5,R6)·효율(R4,R8,R10,
#         R12,R13a,R13b,R14,R15) 그룹에 정책별 배율을 곱해 비중을 조절.
#          ① ES           : 안정×0.0 + 효율×0.0  (필수만,  6키 활성)
#          ② SF_9_EF_1    : 안정×0.9 + 효율×0.1  (전체 16키 활성)
#          ③ SF_7_EF_3    : 안정×0.7 + 효율×0.3  (전체 16키 활성)
#          ④ BL_SF_5_EF_5 : 안정×0.5 + 효율×0.5  (균형, 전체 16키 활성)
#          ⑤ EF_7_SF_3    : 안정×0.3 + 효율×0.7  (전체 16키 활성)
#          ⑥ EF_9_SF_1    : 안정×0.1 + 효율×0.9  (전체 16키 활성)
#       · 커리큘럼 4단계 (Lv1→Lv4, 100% 적재율):
#          Lv1: 4×4(16c) → Lv2: 6×6(36c) → Lv3: 8×8(64c) → Lv4: 10×10(100c)
#          OBS_DIM=79 고정 → model.set_env() 전이학습 호환
#   · 총 학습량: 6 정책 × 125K eps = 750K eps ≈ 48.4M steps
#   · 학습 종료 기준: episodes_per_level[lv] 에피소드 도달 시 callback이 종료
# ══════════════════════════════════════════════════════════════════
CONFIG = {
    "experiment_name"    : "single_bay_6pod_ppo_v16_4policy_50to50",
    "seed"               : GLOBAL_SEED,
    ...
    # ── 보상 가중치 (R1~R15, ★ v16: 안정:효율 50:50 — 계층: 필수/안정/효율) ──
    "rw": {
        "valid"               :    1.0,   # R1  [필수] 유지
        "stack_full"          :  -10.0,   # R2  [필수] 유지
        "overstow"            :  -15.0,   # R3  [필수] 유지
        "order_bonus"         :    2.0,   # R4  [효율] ★ v15: 3.0 → 2.0
        "weight_balance"      :    6.5,   # R5  [안정] ★ v16: 6.0 → 6.5 (안정:효율 = 15:15 정확히 50:50)
        "cog_penalty"         :   -8.5,   # R6  [안정] ★ v16: -8.0 → -8.5 (안정:효율 = 15:15 정확히 50:50)
...
# ── 설정 확인 출력 ──
print("✅ CONFIG loaded (PPO only, v16: 4-policy ES/SF/EF/BL + 계층 가중치 안정:효율=50:50 + 8M steps/policy)")
print()
...
print()
print(f"  보상 구조: v16 — 계층 분류(필수/안정/효율) + 안정:효율 50:50 (Σ|w|=15:15)")
...
# ── ★ v16: 조정 가중치 변경 확인 (v14 대비) ──
print()
print("  ── ★ v16 조정 가중치 (v14 → v16) ──")
print(f"  {'ID':<6} {'Key':<22} {'v14':>8} {'v16':>8} {'배율':>6}")
print("  " + "─" * 56)
_v14_w = {
...
for k, v14 in _v14_w.items():
    v16 = CONFIG["rw"][k]
    ratio = abs(v16 / v14) if v14 != 0 else float('inf')
    print(f"  {_rid[k]:<6} {k:<22} {v14:>+7.2f} {v16:>+7.2f} {ratio:>5.2f}×")
del _env_test, _env_test2, _obs



# ══════════════════════════════════════════════════════════════════
#  ★ v16: 보상 계층(필수/안정/효율) 기반 4-정책 정의 (안정:효율 50:50)
...
def make_phase_weights(base_w: dict, mode: str) -> dict:
    """학습 모드별 보상 가중치 생성 (★ v17: 안정·효율 연속 가중(weighted scaling) 6모드).

    Args:
        base_w: BASE 보상 가중치 (CONFIG["rw"])
        mode  : 다음 중 하나
          [v17 신규 — 안정:효율 비중 스케일링]
          'ESSENTIAL_ONLY' — 필수만 (안정×0, 효율×0)
          'SF9_EF1' — 필수 + 안정×0.9 + 효율×0.1
          'SF7_EF3' — 필수 + 안정×0.7 + 효율×0.3
          'SF5_EF5' — 필수 + 안정×0.5 + 효율×0.5  (균형, BL)
          'EF7_SF3' — 필수 + 안정×0.3 + 효율×0.7
          'EF9_SF1' — 필수 + 안정×0.1 + 효율×0.9
          [legacy 호환 — 기존 on/off ablation 모드]
          'ESSENTIAL_STABILITY'(안정×1,효율×0) / 'ESSENTIAL_EFFICIENCY'(안정×0,효율×1)
          'BASE'(안정×1,효율×1) / alias: 'STABILITY_ONLY','EFFICIENCY_ONLY'

    Returns:
        modified weight dict (same keys). 필수 그룹은 항상 원본 유지,
        안정/효율 그룹은 모드별 배율(scale)을 곱한다.
    """
    # ── 모드 → (안정 배율, 효율 배율) 매핑 ──
    _scale = {
        # v17 연속 가중(weighted scaling) 6모드
        "ESSENTIAL_ONLY":       (0.0, 0.0),
        "SF9_EF1":              (0.9, 0.1),
        "SF7_EF3":              (0.7, 0.3),
        "SF5_EF5":              (0.5, 0.5),
        "EF7_SF3":              (0.3, 0.7),
        "EF9_SF1":              (0.1, 0.9),
        # legacy on/off ablation 모드 (하위 호환)
        "ESSENTIAL_STABILITY":  (1.0, 0.0),
        "ESSENTIAL_EFFICIENCY": (0.0, 1.0),
        "BASE":                 (1.0, 1.0),
        # legacy alias
        "STABILITY_ONLY":       (1.0, 0.0),
        "EFFICIENCY_ONLY":      (0.0, 1.0),
    }
    if mode not in _scale:
        raise ValueError(f"Unknown mode: {mode!r}")
    stab_scale, eff_scale = _scale[mode]

    w = dict(base_w)                 # 필수 그룹은 그대로 유지
    for k in STABILITY_KEYS:
        w[k] = base_w[k] * stab_scale
    for k in EFFICIENCY_KEYS:
        w[k] = base_w[k] * eff_scale
    return w


# ── 정책 정의 (★ v17: 6-way — 안정:효율 비중 연속 가중 실험) ──
#   각 정책은 4 라운드(커리큘럼 Lv1→Lv4) 내내 *고정된* 보상 가중치로 학습한다.
#   필수(R1,R2,R3,R7,R9,R11)는 모든 정책 동일. 안정(R5,R6)·효율(R4,R8,R10,
#   R12,R13a,R13b,R14,R15) 그룹에 정책별 배율을 곱해 비중을 조절한다.
#     ① ES         : 안정×0.0 + 효율×0.0  (필수만)
#     ② SF_9_EF_1  : 안정×0.9 + 효율×0.1
#     ③ SF_7_EF_3  : 안정×0.7 + 효율×0.3
#     ④ BL_SF_5_EF_5: 안정×0.5 + 효율×0.5  (균형)
#     ⑤ EF_7_SF_3  : 안정×0.3 + 효율×0.7
#     ⑥ EF_9_SF_1  : 안정×0.1 + 효율×0.9
POLICIES = {
    "ES": {
        "desc":          "Essential-Only (필수 R1,R2,R3,R7,R9,R11 only)",
        "kor":           "① 필수만 (안정×0.0, 효율×0.0)",
        "color":         "#2CA02C",   # green
        "marker":        "D",
        "phase1_rounds": [1, 2],
        "phase2_rounds": [3, 4],
        "phase1_mode":   "ESSENTIAL_ONLY",
        "phase2_mode":   "ESSENTIAL_ONLY",
        "is_baseline":   True,
    },
    "SF_9_EF_1": {
        "desc":          "필수 + 안정×0.9 + 효율×0.1",
        "kor":           "② 안정×0.9, 효율×0.1",
    ...
    "BL_SF_5_EF_5": {
        "desc":          "필수 + 안정×0.5 + 효율×0.5 (균형)",
        "kor":           "④ 안정×0.5, 효율×0.5 (균형)",
        "color":         "#7F7F7F",   # gray
        "marker":        "^",
        "phase1_rounds": [1, 2],
        "phase2_rounds": [3, 4],
...
# ── 정책 검증 출력 (★ v17: 6-정책 안정:효율 비중 가중) ──
_n_rw = len(CONFIG["rw"])
_pol_names = list(POLICIES.keys())
print()
print("  ── ★ v17 안정:효율 비중 6-정책 (전 라운드 고정 가중치) ──")
print(f"  {'정책':<14} {'mode':<16} {'활성수':>8}   {'설명'}")
print("  " + "─" * 78)
for pn, ps in POLICIES.items():
    p_w = make_phase_weights(CONFIG["rw"], ps["phase1_mode"])
    p_active = sum(1 for v in p_w.values() if v != 0)
    print(f"  {pn:<14} {ps['phase1_mode']:<16} ({p_active:2}/{_n_rw})   {ps['kor']}")

print()
print("  ── 정책별 보상 가중치 비교 (0.00 = 비활성) ──")
# 정책별 가중치 dict 사전 계산
_pol_w = {pn: make_phase_weights(CONFIG["rw"], POLICIES[pn]["phase1_mode"])
          for pn in _pol_names}
_hdr = f"  {'R':<6} {'계층':<5} {'Key':<22}" + "".join(f"{pn:>14}" for pn in _pol_names)
print(_hdr)
print("  " + "─" * (33 + 14 * len(_pol_names)))
for k in CONFIG["rw"]:
    row = f"  {R_MAP.get(k, '?'):<6} {TIER_MAP.get(k, '?'):<5} {k:<22}"
    row += "".join(f"{_pol_w[pn][k]:>+14.3f}" for pn in _pol_names)
    print(row)
print("  " + "─" * (33 + 14 * len(_pol_names)))
# 그룹별 Σ|w| 요약 (안정/효율 비중 확인)
def _grp_abssum(wd, keys): return sum(abs(wd[k]) for k in keys)
print(f"  {'':<6} {'':<5} {'안정 Σ|w| (R5,R6)':<22}"
      + "".join(f"{_grp_abssum(_pol_w[pn], STABILITY_KEYS):>+14.3f}" for pn in _pol_names))
print(f"  {'':<6} {'':<5} {'효율 Σ|w| (R4..R15)':<22}"
      + "".join(f"{_grp_abssum(_pol_w[pn], EFFICIENCY_KEYS):>+14.3f}" for pn in _pol_names))
print(f"  {'':<6} {'':<5} {'활성 보상 수':<22}"
      + "".join(f"{sum(1 for v in _pol_w[pn].values() if v != 0):>14d}" for pn in _pol_names))
